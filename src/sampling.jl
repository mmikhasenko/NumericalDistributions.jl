...
function invcdf(d::InterpolatedConstant, u::Real)
    # Use the fast vectorized implementation for a single value
    return fast_invcdf_constant(d, [u])[1]
end
...
function invcdf(d::InterpolatedLinear, u::Real)
    fast_invcdf_linear(d, [u])[1]
end

# Remove custom rand for InterpolatedConstant and InterpolatedLinear to use fast BinnedDensity sampling
...
# --- Shared fast invcdf for InterpolatedConstant ---
